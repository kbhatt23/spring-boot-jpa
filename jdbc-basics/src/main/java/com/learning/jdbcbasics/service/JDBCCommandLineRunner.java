...
public class JDBCCommandLineRunner implements CommandLineRunner{
	...
	private Logger logger = LoggerFactory.getLogger(JDBCCommandLineRunner.class);
	...
	public void run(String... args) throws Exception {
		...
		int dId = 10002;
		logger.info("deleting person by id "+dId);
		personJDBCDAO.deleteByID(dId);
		
		logger.info("creating person with id "+dId);
		Person createPerson = new Person(dId, "auto created", "temp", new Date());
		personJDBCDAO.insert(createPerson);
		
		logger.info("updating person witd id "+dId);
		createPerson.setName("auto updated");
		createPerson.setAddress("updated temp");
		personJDBCDAO.update(createPerson);
		...
		personJDBCDAO.findByAddress(address).forEach(a -> logger.info("Person with address {} is {}",address,a));
		
		String name = "narayan";
		personJDBCDAO.findByAddressAndName(address,name).forEach(a -> logger.info("Person with address {} and name {} is {}",address,name,a));
		logger.info("==========================================");
	}
